...
from agentrl import Agent, Environment, DQNTrainer
...
def main():
    ...
    config.set("agent.algorithm", "DQN")
    config.set("agent.lr", 1e-4)
    config.set("training.max_episodes", 500)
    config.set("training.eval_freq", 50)
    config.set("logging.experiment_name", "cartpole_dqn")
    
    print("=== AgentRL DQN训练示例 ===")
    ...
    agent_config = {
        ...
        "epsilon": config.get("dqn.epsilon"),
        "epsilon_min": config.get("dqn.epsilon_min"),
        "epsilon_decay": config.get("dqn.epsilon_decay"),
        "target_update_freq": config.get("dqn.target_update_freq"),
        "batch_size": config.get("agent.batch_size"),
        "buffer_size": config.get("agent.buffer_size"),
        "learning_starts": config.get("dqn.learning_starts"),
        "train_freq": config.get("dqn.train_freq"),
        "double_dqn": config.get("dqn.double_dqn"),
        "dueling_dqn": config.get("dqn.dueling_dqn"),
        "prioritized_replay": config.get("dqn.prioritized_replay"),
    ...
    print(f"智能体创建成功")
    print(f"使用Double DQN: {agent_config['double_dqn']}")
    print(f"使用Dueling DQN: {agent_config['dueling_dqn']}")
    print(f"使用优先经验回放: {agent_config['prioritized_replay']}")
    print()
    ...
    trainer = DQNTrainer(agent, env, trainer_config)
    ...
    print(f"最终ε值: {agent.agent.epsilon:.4f}")
